    ...
    <div className="px-4 md:px-8 max-w-7xl mx-auto py-4 md:py-6 h-[calc(100vh-114px)] flex flex-col overflow-hidden">
      <div className="mb-6 shrink-0">
        <h1 className="text-3xl font-display font-bold text-foreground mb-2 flex items-center gap-3">
          <span className="bg-[#D0282E]/10 text-[#D0282E] px-3 py-1 rounded-xl text-lg">Food</span>
          Campus Food
        </h1>
        <p className="text-muted-foreground">Order delicious meals from the best vendors around campus.</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto hide-scrollbar pb-6 rounded-2xl">
        {isLoading && page === 1 ? (
          ...
          <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-border/60">
            <h3 className="text-xl font-bold mb-2">No food items found</h3>
            <p className="text-muted-foreground">We couldn't find any food items listed right now.</p>
          </div>
        )}
      </div>
      ...
      <button
        ...
        className="fixed bottom-20 right-5 w-14 h-14 bg-[#D0282E] text-white rounded-full flex items-center justify-center shadow-xl shadow-[#D0282E]/40 hover:shadow-2xl hover:scale-105 active:scale-95 transition-all z-40 group"
      >
